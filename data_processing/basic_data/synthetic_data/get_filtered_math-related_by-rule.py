...
def main():
    in_dir = "data/Education-College-Students"
    in_files = [os.path.join(in_dir, file) for file in os.listdir(in_dir)]
    
    out_dir = "data/Education-College-Students_filtered-by-rule"
    for in_file in in_files:
        if os.path.isfile(in_file):
            print(in_file)
            out_file = os.path.join(out_dir, "filtered-by-rule_" + os.path.basename(in_file))
