...
def main():
    ...
    global model
    model_path = "fasttext_models/model_math-vs-other.bin"
    ...
    for in_dir in in_dirs:
        in_files.extend([os.path.join(in_dir, file_name) for file_name in os.listdir(in_dir) if file_name.startswith("cc")])
    out_dir = "data/cc-en-math_filtered_orig"
    in_files = in_files[:20]
        ...
        with open(in_file, "r", encoding="utf-8") as f:
            ...
            for idx, line in enumerate(f):
                ...
                if label == "__label__math":
                    out.append({"i": idx, "t": "m", "text": orig_text})
                else:
                    out.append({"i": idx, "t": "o", "text": ""})
                if len(out) >= 10:
